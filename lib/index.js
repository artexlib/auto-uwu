...
module.exports = function UwuCore(uwu) {
  ...
  function loadConfig() {
    let settingsFolderPath = path.resolve(__dirname, '../settings');
    let configPath = path.resolve(settingsFolderPath, 'config.json');
  
    try {
      fs.accessSync(settingsFolderPath, fs.constants.F_OK);
    } catch (err) {
    ...
    try {
      ...
      let originalConfigPath = path.resolve(__dirname, './config.json');
  ...
  function saveConfig() {
    let configPath = path.resolve(__dirname, '../settings/config.json');
